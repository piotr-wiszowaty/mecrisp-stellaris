...
@ -----------------------------------------------------------------------------
	defword Flag_visible, "16flash!" @ Writes 16 Bytes at once into Flash.
...
1:	ldr r1, =FLASH_SR+2
	...
	bx lr
@ ------
	defword Flag_visible, "flashpageerase" @ ( Addr -- )
...
2:	pop {r0, r1, r2, r3, pc}
	...
	.ltorg
@ ------
	defword Flag_visible, "eraseflash" @ ( -- )
	@ Löscht den gesamten Inhalt des Flashdictionaries.
        ldr r0, =FLASH_DICTIONARY_START
eraseflash_intern:
	cpsid i
	ldr r1, =FLASH_DICTIONARY_END
	ldr r2, =0xFFFF
...
2:	adds r0, #2
	...
	bl restart
@ ------
	defword Flag_visible, "eraseflashfrom" @ ( Addr -- )
