...
@ Small M0 cores have no division. Large A9 or A15 cores have no division either, for some crazy reasons.
	...
	.endif
@ -----------------------------------------------------------------------------
	defword Flag_inline|Flag_opcodierbar_Plusminus, "+" @ ( x1 x2 -- x1+x2 )
				@ Adds x1 and x2
	ldm psp!, {r0}
	adds tos, r0
	bx lr
...
@ -----------------------------------------------------------------------------
	...
	.ifndef m0corei		@ Opcode with 12-bit encoded constant only available on M3/M4
	.hword 0x0600
	.hword 0xF1B6
	.endif			@ Opcode subs tos, tos, #imm12
@ -----------------------------------------------------------------------------
	defword Flag_foldable_1|Flag_inline, "1-" @ ( u -- u-1 )
...
@ -----------------------------------------------------------------------------
	...
	bx lr
@ -----------------------------------------------------------------------------
	defword Flag_foldable_1|Flag_inline, "2-" @ ( u -- u-1 )
...
@ -----------------------------------------------------------------------------
	...
	bx lr
@ -----------------------------------------------------------------------------
	defword Flag_foldable_1|Flag_inline, "negate" @ ( n1 -- -n1 )

	rsbs tos, tos, #0
	bx lr
@ -----------------------------------------------------------------------------
	defword Flag_foldable_1|Flag_inline, "abs" @ ( n1 -- |n1| )
...
@ -----------------------------------------------------------------------------
	...
	.else
@ -----------------------------------------------------------------------------
	defword Flag_foldable_2|Flag_inline, "/" @ ( n1 n2 -- n1/n2 )
	...
	.endif
@ -----------------------------------------------------------------------------
	defword Flag_inline|Flag_opcodierbar_Rechenlogik, "*" @ ( u1|n1 u2|n2 -- u3|n3 )
...
@ -----------------------------------------------------------------------------
	defword Flag_foldable_1|Flag_inline, "2*" @ ( n -- n*2 )
