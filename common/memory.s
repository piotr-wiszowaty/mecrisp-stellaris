...
@------------------------------------------------------------------------------
  defword Flag_visible, "move"  @ Move some bytes around. This can cope with overlapping memory areas.
...
@------------------------------------------------------------------------------
  defword Flag_visible, "fill"  @ Fill memory with given byte.
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "!" @ ( x 32-addr -- )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline, "h@" @ ( 16-addr -- x )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "h!" @ ( x 16-addr -- )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline, "c@" @ ( 8-addr -- x )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "c!" @ ( x 8-addr -- )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "bis!" @ ( x 32-addr -- )  Set bits
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "bic!" @ ( x 32-addr -- )  Clear bits
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "xor!" @ ( x 32-addr -- )  Toggle bits
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Spezialfall, "bit@" @ ( x 32-addr -- Flag )  Check bits
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "hbis!" @ ( x 16-addr -- )  Set bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "hbic!" @ ( x 16-addr -- )  Clear bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ldrh r2, [tos] @ Alten Inhalt laden
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "hxor!" @ ( x 16-addr -- )  Toggle bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ldrh r2, [tos] @ Alten Inhalt laden
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Spezialfall, "hbit@" @ ( x 16-addr -- Flag )  Check bits
  @ Prüft, ob Bits in der Speicherstelle gesetzt sind
@ -----------------------------------------------------------------------------
  ldm psp!, {r0}  @ Bitmaske holen
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "cbis!" @ ( x 8-addr -- )  Set bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "cbic!" @ ( x 8-addr -- )  Clear bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ldrb r2, [tos] @ Alten Inhalt laden
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Speicherschreiben, "cxor!" @ ( x 8-addr -- )  Toggle bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ldrb r2, [tos] @ Alten Inhalt laden
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_opcodierbar_Spezialfall, "cbit@" @ ( x 8-addr -- Flag )  Check bits
  @ Prüft, ob Bits in der Speicherstelle gesetzt sind
@ -----------------------------------------------------------------------------
  ldm psp!, {r0}  @ Bitmaske holen
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_foldable_1, "cell+" @ ( x -- x+4 ) 
@ -----------------------------------------------------------------------------
  adds tos, #4
  bx lr

@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_foldable_1, "cells" @ ( x -- 4*x ) 
@ -----------------------------------------------------------------------------
  lsls tos, #2
  bx lr
