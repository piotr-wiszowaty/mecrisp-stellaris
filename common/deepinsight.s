...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "hex." @ Print an unsigned number in Base 16, independent of number subsystem.
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "h.s"  @ Prints out data stack, uses unsigned hexadecimal snumbers.
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "u.s"  @ Prints out data stack, uses unsigned numbers.
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, ".s"  @ Prints out data stack, uses signed numbers.
...
1:@ Berechne den Stackfüllstand  Calculate number of elements on datastack
  ...
  cmp psp, #32
	...
	@ Ausgabepuffer vorbereien  Prepare string output buffer
2:	muls r3, r0
	lsrs r3, r0, #9
	ldrb r0, [r7, r0]
	...
	@ Save Base and switch to decimal for stack fill gauge
3:	ldr r2, =base
	ldr r0, [r2]
	push {r0, r1}
	...
	@ Fetch stack element directly
1:	ldr r0, [r2]
	push {r1, r2}
	pushda r0
	blx r4			@ Doesn't save registers !
	pop {r1, r2}
...
2:	write " TOS: "
	...
	.ifdef debug
@ -----------------------------------------------------------------------------
  defword Flag_visible, "dump" @ ( addr u -- ) Prints some memory locations beginning with given adress
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, ".regs" @ ( -- ) Prints register contents
dotregs:
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "words" @ Print list of words with debug information
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "unused" @ Bytes free for compilation in current memory area
unused:
  push {lr}
  bl flashvarhere
