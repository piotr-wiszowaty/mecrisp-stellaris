...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "@" @ ( 32-addr -- x )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "!" @ ( x 32-addr -- )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "h@" @ ( 16-addr -- x )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "h!" @ ( x 16-addr -- )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "c@" @ ( 8-addr -- x )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "c!" @ ( x 8-addr -- )
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "bis!" @ ( x 32-addr -- )  Set bits
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "bic!" @ ( x 32-addr -- )  Clear bits
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "xor!" @ ( x 32-addr -- )  Toggle bits
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "bit@" @ ( x 32-addr -- Flag )  Check bits
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "hbis!" @ ( x 16-addr -- )  Set bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "hbic!" @ ( x 16-addr -- )  Clear bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ldrh r2, [tos] @ Alten Inhalt laden
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "hxor!" @ ( x 16-addr -- )  Toggle bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ldrh r2, [tos] @ Alten Inhalt laden
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "hbit@" @ ( x 16-addr -- Flag )  Check bits
  @ Prüft, ob Bits in der Speicherstelle gesetzt sind
@ -----------------------------------------------------------------------------
  ldm psp!, {r0}  @ Bitmaske holen
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "cbis!" @ ( x 8-addr -- )  Set bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ...
  bx lr
    ...
    pop {pc}
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "cbic!" @ ( x 8-addr -- )  Clear bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ldrb r2, [tos] @ Alten Inhalt laden
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "cxor!" @ ( x 8-addr -- )  Toggle bits
  @ Setzt die Bits in der Speicherstelle
@ -----------------------------------------------------------------------------
  ldm psp!, {r0, r1} @ X is the new TOS after the store completes.
  ldrb r2, [tos] @ Alten Inhalt laden
...
@ -----------------------------------------------------------------------------
  defword Flag_inline|Flag_allocator, "cbit@" @ ( x 8-addr -- Flag )  Check bits
  @ Prüft, ob Bits in der Speicherstelle gesetzt sind
@ -----------------------------------------------------------------------------
  ldm psp!, {r0}  @ Bitmaske holen
