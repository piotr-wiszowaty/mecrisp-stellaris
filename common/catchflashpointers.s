...
@ This runs one time after Reset, no registers are saved here.
	...
	pushdatos
	ldr tos, =core_dictionary_start  @ Hier fängt es an.  Start at the beginning
	ldr r5,  =RAM_DICTIONARY_END     @ Fürs Abzählen des Variablenplatzes  Variables start at the end of RAM dictionary
...
SucheFlashPointer_Hangelschleife:
	...
	adds r0, tos, #6
	bl skipstring
	@ r0 points to start of code of current definition
	bl suchedefinitionsende @ Advance pointer to end of code. This is detected by "bx lr" or "pop {pc}" opcodes.
	@ r0 now points to the location of the initialisation at the end of code of current definition.
...
Sucheflashpointer_Speicherbelegung_fertig:
  ...
  @ Time to search the Dictionarypointer !
	...
	ldr r0, =FLASH_DICTIONARY_END
	ldr r1, =FLASH_DICTIONARY_START
