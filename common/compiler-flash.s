...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "setflags" @ ( x -- )
...
@ -----------------------------------------------------------------------------
  defword Flag_visible|Flag_foldable_1, "aligned" @ ( c-addr -- a-addr )
@ -----------------------------------------------------------------------------
  movs r0, #1
  ands r0, tos
  adds tos, r0
...
@ -----------------------------------------------------------------------------
	defword Flag_visible, "string," @ ( c-addr length -- )
...
4:	@ One character left
	...
	pop {r0, r1, r2, pc}
@------------------------------------------------------------------------------
  defword Flag_visible, "allot" @ Erhöht den Dictionaryzeiger, schafft Platz !  Advance Dictionarypointer and check if there is enough space left for the requested amount.
...
allot_ok:				@ Everything is fine, just allot it!
	str r1, [r0]
	bx lr
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "forgetram"
  push {lr}
  bl compiletoram
    @ Dictionarypointer ins RAM setzen
    @ Set dictionary pointer into RAM first
    ldr r0, =Dictionarypointer
    ldr r1, =RAM_DICTIONARY_START
    str r1, [r0]

    @ Fadenende fürs RAM vorbereiten
    @ Set latest for RAM
    ldr r0, =Fadenende
    ldr r1, =core_dictionary_start
    str r1, [r0]
  pop {pc}

@ -----------------------------------------------------------------------------
  defword Flag_visible, "compiletoram?"
@ -----------------------------------------------------------------------------
  pushdaconst 0
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "compiletoflash"
compiletoflash:
@ -----------------------------------------------------------------------------
  @ Prüfe, ob der Dictionarypointer im Ram oder im Flash ist:
  ldr r0, =Dictionarypointer
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "variable" @ ( n -- )
@ -----------------------------------------------------------------------------
  pushdaconst 1
  b.n nvariable

@ -----------------------------------------------------------------------------
  defword Flag_visible, "2variable" @ ( d -- )
@ -----------------------------------------------------------------------------
  pushdaconst 2
  b.n nvariable

@------------------------------------------------------------------------------
  defword Flag_visible, "nvariable" @ ( Init-Values Length -- )
...
@------------------------------------------------------------------------------
  defword Flag_visible, "buffer:" @ ( Length -- )
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "dictionarystart"
...
@ -----------------------------------------------------------------------------
  ...
  cmp r0, r1
  bhs 1f
  ldr tos, =core_dictionary_start @ Befinde mich im Flash mit Backlinks. Muss beim CoreDictionary anfangen:        In Flash: Start with core dictionary.
  bx lr
...
@  ite lo
@    ldrlo r2, =core_dictionary_start @ Befinde mich im Flash mit Backlinks. Muss beim CoreDictionary anfangen:        In Flash: Start with core dictionary.
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "dictionarynext" @ ( address -- address flag )
...
@ -----------------------------------------------------------------------------
  defword Flag_visible, "skipstring"
@ -----------------------------------------------------------------------------
...
@------------------------------------------------------------------------------
  defword Flag_visible|Flag_variable, "hook-find" @ ( -- addr )
